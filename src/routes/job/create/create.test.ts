...
test('all day of month every minute of every hour', () => {
    expect(getCronString({
        type: "dom",
        dayOfWeek: [],
        month: [...placeholderArray.keys().take(12).map(i => i + 1)],
        day: [...placeholderArray.keys().take(31).map(i => i + 1)],
        hour: [...placeholderArray.keys().take(24)],
        minute: [...placeholderArray.keys().take(60)]
    })).toBe("* * * * *")
})
